#!/usr/bin/env node
"use strict";
// model-hub ZCode patch v1.2.1-adapted for ZCode Desktop 3.12.3.7463
// Based on CSSZYF/zcode-modelhub-patch v1.2.1; renderer anchors re-derived for 3.12.3.
const fs = require("fs");
...
if (!resourcesDir) die("未找到 ZCode 安装目录。请手动指定：node patch-core-adapted.js \"ZCode安装目录下的resources文件夹\"");
...
const RENDER_REL = "out/renderer/assets/styles-ou2or4Yg.js";
...
// ---- 渲染层锚点：M5 组件内 vRt 模型列表的调用点（3.12.3 中全文件唯一）----
// 注入位置在 M5 作用域内，可访问: e(provider) t(onSave) n(onAddPersonalModel) se(保存包装器)
// W(当前模型列表) E(baseUrl) O(apiKey) w(apiFormat)
const VRT_START = "(0,$.jsx)(vRt,{";
...
try {
  ...
  if (p == null || m == null || r == null) die("目标文件缺失，ZCode 版本可能不兼容此适配版补丁");
  p = p.toString("utf8"); m = m.toString("utf8"); r = r.toString("utf8");
  if (p.includes("modelhubFetchModels") || r.includes("__mhPick")) die("检测到已安装过补丁，如需重装请先还原（node patch-core-adapted.js \"resources目录\" --restore）");

  if (!p.includes(PRELOAD_ANCHOR)) die("preload 锚点不匹配，ZCode 版本不兼容");

  // 定位渲染层注入点：M5 内 vRt 调用（要求全文件唯一且结构符合预期）
  if (r.split(VRT_START).length - 1 !== 1) die("渲染层锚点(vRt调用)不唯一或未找到，ZCode 版本不兼容");
  const vs = r.indexOf(VRT_START);
  const ve = r.indexOf(VRT_END, vs);
  if (ve < 0) die("渲染层锚点(vRt调用)结束标记未找到");
  const VRT_ANCHOR = r.slice(vs, ve + VRT_END.length);
  if (!VRT_ANCHOR.includes("models:W") || !VRT_ANCHOR.includes("onAddModel:je") || !VRT_ANCHOR.includes("onModelCommit:Oe")) die("渲染层锚点内容校验失败，ZCode 版本不兼容");
  ...
  m += MAIN_HANDLERS;
  const BTN_ROW = "(0,$.jsxs)(`div`,{className:`flex justify-end gap-2`,children:[" + BTN_PULL + "," + BTN_HEADERS + "]})";
  const VRT_WRAP = "(0,$.jsxs)(`div`,{className:`space-y-2`,children:[" + BTN_ROW + "," + VRT_ANCHOR + "]})";
  ...
  console.log("\n[√] 补丁适配版安装完成！重启 ZCode：设置 → 模型供应商 → 编辑渠道，即可看到「拉取模型」「请求头模拟」按钮。");
} catch (e) {
